...
🎯 Ispol'zuyte komandu <code>status-ai</code> dlya proverki raboty"
    ...
    # [ИСПРАВЛЕНО] УДАЛЕН ПРЕЖДЕВРЕМЕННЫЙ ВЫХОД
}

#==============================================================================
# ФУНКЦИЯ УСТАНОВКИ БАЗОВЫХ ЗАВИСИМОСТЕЙ (была частью main)
#==============================================================================
install_base_dependencies() {
    echo_t "[$(date '+%Y-%m-%d %H:%M:%S')] Proveryayu i ustanavlivayu zavisimosti..."
...
main() {
    ...
    install_base_dependencies # [ИСПРАВЛЕНО] ВЫЗОВ ФУНКЦИИ УСТАНОВКИ ЗАВИСИМОСТЕЙ
    echo ""
...
Vremya: $(date '+%Y-%m-%d %H:%M:%S')"
    ...
    # [ИСПРАВЛЕНО] ВЫЗОВ ОСНОВНОГО БЛОКА УСТАНОВКИ
    install_dependencies
}
